...
function TopicCard({ topic, onOpen }: { topic: HotTopic; onOpen: () => void }) {
  return (
    <motion.div
      ...
      className="bg-black/60 backdrop-blur-md p-6 rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 border border-gray-700/50 hover:border-teal-500/50"
    >
      ...
      <div className="flex gap-2">
        <Button
          onClick={onOpen}
          className="flex-1 inline-flex items-center justify-center px-4 py-2 border border-teal-500 text-teal-300 hover:bg-teal-500/20 rounded-md text-sm font-medium transition-colors cursor-pointer"
        >
          Read Here
        </Button>
        <a
          ...
          className="flex-1 inline-flex items-center justify-center px-4 py-2 border border-teal-500 text-teal-300 hover:bg-teal-500/20 rounded-md text-sm font-medium transition-colors"
        >
    ...
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="bg-black/95 backdrop-blur-xl text-white w-[95vw] h-[90vh] max-w-[95vw] !max-w-[95vw] p-6 rounded-2xl border-2 border-gray-700 shadow-2xl overflow-hidden">
        <DialogHeader className="p-8 pb-4 border-b border-gray-800 sticky top-0 bg-black/95 z-10 md:w-[1000px]">
          <DialogTitle className="text-4xl font-bold text-teal-400">
            {topic.title}
          </DialogTitle>
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(100vh-120px)] custom-scrollbar">
          <div className="prose prose-invert prose-headings:text-white prose-a:text-teal-400 max-w-none">
          ...
          <div className="mt-10 flex items-center gap-8">
            ...
            <Button
              ...
              className={`flex items-center gap-3 text-lg ${
                isUpvoted
                  ? "text-green-400"
                  : "text-gray-300 hover:text-teal-400 cursor-pointer"
              }`}
              ...
              {topic.sourceUrl.map((url, index) => (
                <li key={index}>
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-teal-400 hover:text-teal-200 flex items-center gap-3 text-lg"
                  >
...
export default function VLSIPulse() {
  ...
  return (
    ...
    >
      <Header />
      <div className="relative flex flex-col grow p-4 md:p-8">
